...
A sliding item contains an item that can be dragged to reveal buttons. It requires an [item](item.mdx) component as a child. All options to reveal should be placed in the [item options](item-options.mdx) element.
...
By default if an icon is placed with text in the [item option](item-option.mdx), it will display the icon on top of the text, but the icon slot can be changed to any of the following to position it in the option.
