...
:::note
Ionic Framework components are **not all** Shadow DOM components. If the component is a Shadow DOM component, there will be a badge in the top right of its [component documentation](/docs/components). An example of a Shadow DOM component is the [button component](/docs/api/button).
:::
...
The above shows two parts: `placeholder` and `icon`. See the [select documentation](/docs/api/select#css-shadow-parts) for all of its parts.
...
All exposed parts for an Ionic Framework component can be found under the CSS Shadow Parts heading on its API page. To view all components and their API pages, see the [Component documentation](/docs/components).

In order to have parts a component must meet the following criteria:

- It is a [Shadow DOM](../reference/glossary.md#shadow) component. If it is a [Scoped](../reference/glossary.md#scoped) or Light DOM component, the child elements can be targeted directly. If a component is Scoped or Shadow, it will be listed by its name on its [component documentation page](/docs/components).
