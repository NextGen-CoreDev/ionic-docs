...
4. Remove any usage of the `open` method. To present the datetime in an overlay, place it inside of an `ion-modal` or an `ion-popover` component. See the [ion-datetime Usage Examples](/docs/api/datetime#usage) for more information.

5. Remove any usage of the `displayFormat` or `displayTimezone` properties. To parse the UTC string provided in the payload of the `ionChange` event, we recommend using [date-fns](https://date-fns.org/). See the [ion-datetime Parsing Dates Documentation](/docs/api/datetime#parsing-dates) for examples.
...
`ion-modal` now uses the Shadow DOM. Update any styles targeting the internals of `ion-modal` to use either the [ion-modal CSS Variables](/docs/api/modal#css-custom-properties) or the [ion-modal CSS Shadow Parts](/docs/api/modal#css-shadow-parts):
...
`ion-popover` now uses the Shadow DOM. Update any styles targeting the internals of `ion-popover` to use either [ion-popover CSS Variables](/docs/api/popover#css-custom-properties) or the [ion-popover CSS Shadow Parts](/docs/api/popover#css-shadow-parts):
